...
class ConfirmTransfers extends Component {
  ...
  bindDuplexRequests() {
    this.props.duplex.on('setTransfersConfirmationLoading', isLoading => this.setState({ isLoading }))
    this.props.duplex.on('setTransfersConfirmationError', error => this.setState({ error }))
  }
        ...
        ? <div className="container">
            ...
            <div className={this.state.error ? "row mt-4" : "row mt-12"}>
              <div className="col-6 pr-5 pl-5">
                <button onClick={() => this.props.onReject(this.props.transfer)} className="btn btn-border-blue text-sm text-bold btn-big">
                  Reject
                      </button>
              </div>
              <div className="col-6 pr-5 pl-5">
                <button onClick={() => this.props.onConfirm(this.props.transfer)} className="btn btn-blue text-sm text-bold btn-big">
                  Confirm
                      </button>
              </div>
            </div>
