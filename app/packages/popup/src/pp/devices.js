    ...
    try{
        ...
        for ( const transaction of transactions ) {
            if ( transaction.currentIndex === 0 ) {
                try{
                    const message = await getMessage(transaction.hash);//JSON.parse(trytesToAscii(transaction.signatureMessageFragment.substring(0,transaction.signatureMessageFragment.length-1)).replace(/\0.*$/g,''));
                    console.log(message);
